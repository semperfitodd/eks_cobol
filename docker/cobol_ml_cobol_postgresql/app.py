...
def insert_file(cursor, filepath):
    ...
    with open(filepath, "r") as f:
        for line in f:
            ...
            try:
                ...
                print(f"Inserting: {date}, {description}, {amount}")
                cursor.execute(
    ...
    print(f"Inserted {inserted} of {total} lines from {os.path.basename(filepath)}")
    return inserted
...
def main():
    ...
    files = sorted(f for f in os.listdir(DATA_DIR) if f.endswith(".tsv"))
